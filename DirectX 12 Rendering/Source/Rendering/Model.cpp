...
void Model::Initialize(Device* pDevice, const std::string& PathToModel)
{
	...
	for (uint8_t i = 0; i < m_ConstBuffers.size(); i++) 
	{
		m_ConstBuffers.at(i) = std::make_unique<ConstantBuffer<cbPerObject>>();
		m_ConstBuffers.at(i)->Create(pDevice, &m_cbDatas.at(i));
	}
...
{
	if (pNode->Mesh && pNode->Mesh->Primitives.size() > 0)
	{
		UpdateWorld();
		...
		//m_cbData.WVP = XMMatrixTranspose(currentMatrix * m_WorldMatrix * pCamera->GetViewProjection());
		m_cbData.WVP = XMMatrixTranspose(currentMatrix * pCamera->GetViewProjection());
