...
float4 main(ScreenQuadOutput pin) : SV_TARGET
{
    ...
    float4 positionWS = mul(posCS, InversedProjection * InversedView);
    float3 outPosition = positionWS.rgb / positionWS.w;
    
    //float3 positions = PositionTexture.Load(int3(position.xy, 0)).rgb;
    float3 positions = PositionTexture.Load(int3(outPosition.xy, 0)).rgb;
    float3 albedo = BaseColorTexture.Load(int3(position, 0)).xyz;
    ...
    //float3 output = (ambient.rgb + Lo) * albedo.rgb;
